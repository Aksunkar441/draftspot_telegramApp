...
backend/
  app/
    ...
    services/
      ...
      reminder_service.py       — фоновый цикл напоминаний за 2 часа до игры
    bot/
...
**Регистрация** происходит в боте, а не в Mini App: `/start` → имя → возраст → одно или несколько фото (`file_id` Telegram, без загрузки на свой сервер) → описание (можно пропустить). Город пока хардкодится как «Тараз» — поле `city` в модели уже есть, чтобы потом добавить выбор без миграции.
...
**Лента** (`/api/events/feed`) отдаёт открытые события, кроме тех, что создал сам пользователь или на которые он уже подавал заявку. Фронтенд листает массив локально (курсор в Pinia) — отдельный API-запрос на каждый свайп не нужен.
...
**Напоминания** — фоновый `asyncio`-цикл (`reminder_service.reminder_loop`), который раз в минуту ищет события, начинающиеся в ближайшие 2 часа, и шлёт сообщение капитану и всем принятым игрокам, помечая `reminder_sent = true`, чтобы не дублировать. Для продакшена это стоит вынести в APScheduler/Celery beat — текущий вариант рассчитан на один процесс.
